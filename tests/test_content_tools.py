...
class MinecraftContentToolTests(unittest.TestCase):
    ...
    def test_default_generated_content_uses_continuity_works_identity(self):
        with tempfile.TemporaryDirectory() as td:
            cap = StructureCapability(td)

            book = cap.minecraft_book_generate({
                "target_version": "1.20.5", "title": "Generated Notes", "pages": ["Entry"]
            })
            written = book["item_stack"]["components"]["minecraft:written_book_content"]
            self.assertEqual(written["author"], "Continuity Works")

            loot = cap.minecraft_loot_table_generate({
                "target_version": "1.21", "items": [{"id": "minecraft:stone"}]
            })
            self.assertEqual(loot["resource_id"], "continuityworks:generated_loot")
            self.assertEqual(loot["path"], "data/continuityworks/loot_table/generated_loot.json")

            recipe = cap.minecraft_recipe_generate({
                "target_version": "1.21", "type": "crafting_shapeless",
                "ingredients": ["minecraft:stone"], "result": "minecraft:cobblestone",
            })
            self.assertEqual(recipe["resource_id"], "continuityworks:generated_recipe")
            self.assertEqual(recipe["path"], "data/continuityworks/recipe/generated_recipe.json")

            advancement = cap.minecraft_advancement_generate({
                "target_version": "1.21",
                "criteria": {"generated": "minecraft:impossible"},
                "display": {"title": "Generated Advancement"},
            })
            self.assertEqual(advancement["resource_id"], "continuityworks:generated_advancement")
            self.assertEqual(
                advancement["json"]["display"]["description"],
                {"text": "Generated by Continuity Works"},
            )

            tag = cap.minecraft_tag_generate({
                "target_version": "1.21", "values": ["minecraft:stone"]
            })
            self.assertEqual(tag["resource_id"], "continuityworks:generated_tag")

            manifest = cap.minecraft_datapack_manifest_generate({"target_version": "1.21"})
            self.assertEqual(manifest["json"]["pack"]["description"], "Generated by Continuity Works")

            package = cap.minecraft_content_package_generate({"target_version": "1.21"})
            self.assertEqual(package["package_id"], "continuityworks:generated_package")
            self.assertEqual(
                package["artifacts"]["pack_mcmeta"]["json"]["pack"]["description"],
                "Continuity Works package continuityworks:generated_package",
            )
