...
def _error_response() -> dict:
    return {"description": "Request rejected by the StructureSmith public validation boundary.", "content": {"application/json": {"schema": {"type": "object", "required": ["error"], "properties": {"error": {"type": "string"}, "message": {"type": "string"}}}}}}
...
def openapi_document(base_url: str | None = None) -> dict:
    ...
    paths = {
        "/v1/health": {"get": _get_operation("health", "Check StructureSmith API health and build identity.")},
        ...
        "/v1/tools/{tool_name}": {"get": {**_get_operation("toolContract", "Retrieve exactly one StructureSmith tool contract/schema."), "parameters": [{"name": "tool_name", "in": "path", "required": True, "schema": {"type": "string"}}]}},
        ...
        "/v1/resume": {"post": _post_operation("resume", "Resume a previously created StructureSmith snapshot.", schema={"type": "object", "required": ["snapshot_id"], "properties": {"snapshot_id": {"type": "string"}}, "additionalProperties": False})},
        "/openapi.json": {"get": _get_operation("openapi", "Retrieve this OpenAPI 3.1 document.")},
        "/.well-known/structuresmith.json": {"get": _get_operation("structuresmithDiscovery", "Retrieve absolute StructureSmith machine-discovery metadata.")},
    }
    ...
    return {"openapi": "3.1.0", "info": {"title": "StructureSmith Capability API", "version": installed_version(), "description": "Executable public HTTP boundary for StructureSmith capabilities. GitHub Pages is a separate static frontend."}, "servers": [{"url": base, "description": "Canonical StructureSmith executable API for this service instance."}], "paths": paths, "x-structuresmith": {"api_version": "v1", "tool_schema_version": schema_version, "frontend": canonical_frontend_url(), "api": base, "tool_catalog": f"{base}/v1/tools", "compact_tool_index": f"{base}/v1/tools/index", "tool_contract": f"{base}/v1/tools/{{tool_name}}", "presets": f"{base}/v1/presets", "resolver": f"{base}/v1/resolve", "health": f"{base}/v1/health", "serviceability": f"{base}/v1/serviceability", "discovery": f"{base}/.well-known/structuresmith.json", "progressive_disclosure": True, "public_gate": "PUBLIC_SERVICEABILITY"}}
...
def discovery_document(capability: StructureCapability, base_url: str | None = None) -> dict:
    ...
    return {"schema_version": "1.1", "name": "StructureSmith", "description": "Machine discovery for the executable StructureSmith API. The GitHub Pages origin is a static frontend only.", "frontend": canonical_frontend_url(), "api": base, "build": identity, "endpoints": {"health": f"{base}/v1/health", "serviceability": f"{base}/v1/serviceability", "tools": f"{base}/v1/tools", "compact_tools": f"{base}/v1/tools/index", "presets": f"{base}/v1/presets", "resolver": f"{base}/v1/resolve", "openapi": f"{base}/openapi.json", "discovery": f"{base}/.well-known/structuresmith.json"}, "capabilities": [{"name": tool["name"], **tool.get("x-structuresmith", {}).get("publication", {})} for tool in catalog.get("tools", [])]}
...
def _published_contract(capability: StructureCapability, name: str, base_url: str) -> dict:
    ...
    if published is not None:
        contract["x-structuresmith"] = published.get("x-structuresmith", {})
    return contract
...
def _published_index(capability: StructureCapability, base_url: str, group: str | None = None) -> dict:
    ...
    catalog = published_tool_catalog(tool_catalog(), capability, base_url)
    publication = {item["name"]: item.get("x-structuresmith", {}).get("publication", {}) for item in catalog.get("tools", [])}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _invoke_public(self, spec, body: dict | None = None):
        if self.capability is None: raise RuntimeError("StructureSmith capability has not been initialized")
    ...
    def do_GET(self):
        try:
            ...
            if path == "/.well-known/structuresmith.json": return self._send(200, discovery_document(self.capability, base))
...
def serve(project_root=".", host="127.0.0.1", port=8787):
    ...
    print(f"StructureSmith Capability API listening on http://{host}:{port}")
