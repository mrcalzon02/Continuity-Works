# Continuity Works Frontend Architecture

## Product boundary

**Continuity Works** is both the repository identity and the interactive structural-development workbench/API surface. The browser client presents the reusable structural-reasoning capabilities without creating a parallel generation authority.
...
The interface exposes optional dropdown groups for Continuity Works Generator, Purpose Model, Auditor, Repair, Renderer, and Validator.

The **Continuity Works Renderer** module is local/client functionality. Selecting it controls how the browser presents returned data; it does not request paid visual-generation work from the Continuity Works API host.
