...
All Continuity Works compatibility work is an **additive overlay**. Compatibility must extend an upstream system without replacing, deleting, disabling, clearing, or taking ownership of that system's native behavior.
...
Forbidden operations include replacing existing entries, overriding native generators, deleting or clearing existing tables, disabling native behavior, or making Continuity Works an exclusive replacement for an upstream system.
...
Continuity Works may inspect that source to infer how supplemental assets should fit the native street, highway, railway, building-part, palette, and selector systems. The source is a compatibility reference; Continuity Works does not replace those systems.

### Streets

Lost Cities has a native hierarchical street model with `PRIMARY`, `SECONDARY`, and `TERTIARY` planned road types and cardinal connection metadata. Continuity Works road compatibility should therefore contribute additional compatible road parts or selector/table entries while preserving the native street planner and its existing entries.
...
The Continuity Works strict urban road cross-section remains `5 padding | 6 roadbed | 5 padding`, which is exactly 16 blocks wide and can therefore be adapted cleanly to a chunk-width road part without replacing Lost Cities' planner.
...
Continuity Works highway compatibility should add compatible part candidates, palettes, or optional selector entries to those existing families. It must not disable or supplant the Lost Cities highway planner.
...
Continuity Works railway compatibility must therefore be expressed as additional compatible rail parts, station variants, palettes, or selector/table entries. It must not replace the native railway grid, station-placement logic, or highway-conflict handling.
...
```json
{
  ...
  "additions": {
    "parts": ["continuity_works:..."],
    "selectors": ["continuity_works:..."],
    "table_entries": ["continuity_works:..."]
  }
}
```
...
Static source analysis can establish a strong compatibility hypothesis and can validate that Continuity Works output follows the upstream model. It cannot certify runtime compatibility by itself. Fresh-world testing remains required for actual placement, selector behavior, cross-mod interactions, terrain handling, and version-specific execution.
