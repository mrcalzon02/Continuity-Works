...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke-test a running StructureSmith HTTP API."); parser.add_argument("base_url"); args = parser.parse_args(); base = args.base_url.rstrip("/")
    health = request(base, "GET", "/v1/health")
    if not health.get("ok") or health.get("service") != "StructureSmith": raise RuntimeError("/v1/health did not report an identified healthy StructureSmith service")
    ...
    for tool in catalog["tools"]:
        publication = tool.get("x-structuresmith", {}).get("publication", {})
        if publication.get("http_route") != "ready" or not publication.get("canonical_endpoint"): raise RuntimeError(f"{tool.get('name')} lacks public route metadata")
    ...
    if discovery.get("api", "").rstrip("/") != base: raise RuntimeError("discovery metadata does not identify the running API base")
    ...
    print(json.dumps({"ok": True, "base_url": base, "commit": health.get("commit"), "tool_count": len(catalog["tools"]), "openapi": spec["openapi"], "public_serviceability": serviceability.get("status"), "capability_call": {"route": "/v1/infrastructure/layout", "engine": layout["engine"], "fitness": layout.get("fitness", {}).get("status")}}, indent=2)); return 0
